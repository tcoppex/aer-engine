...
const aer::U32 kBufferSize = 1024u;
char s_buffer[kBufferSize];
...
bool Program::create(ShaderView* shader) {
...
bool Program::link() {
  ...
  bDirty_ = (GL_FALSE == status);

  if (bDirty_) {
    glGetProgramInfoLog(id_, kBufferSize, nullptr, s_buffer);
    aer::Logger::Get().error(s_buffer);
    return false;
  }
...
void Program::add_shader(ShaderView* shader) {
...
UniformLocation_t Program::uniform_location(const char *name) {
  ...
  AER_DEBUG_CODE(
    if (!loc.is_valid()) {
      std::string n(name);
      AER_WARNING("Invalid uniform name : \""+n+"\"");
    }
  )
  return loc;
}
